...
type AutomationRunStep struct {
    ...
    CommandID *devices.CommandID // only present when an actual Command record exists
...
Define named constants for all status/source/kind literals above. Nullable fields reflect lifecycle states, not undocumented options: pending/not-attempted Steps have no reserved ID or start time; attempting a Step writes a reserved ID and start time; only existing Command evidence populates `CommandID`. `satisfied` and `dispatched` require their matching Command status and outcome. A pre-creation validation failure has no Command ID or outcome. An interrupted Step may still expose its linked Command's actual result without changing the Run's interrupted status.
...
Ownership: `internal/modules/devices/model.go`, `command.go`, its API interface/callers, and tests. No external Command HTTP input gains a caller-chosen ID.
...
Add `Service.ValidateCommand(ctx context.Context, input CommandInput) (CommandParameters, error)` in the same owner. It performs the same ID/Operation/JSON/catalog validation and returns an owned copy of normalized parameters, without generating IDs, writing records, checking temporary availability/enablement, or dispatching. Factor common validation; do not duplicate catalog rules. Execution still rereads current support, routing, enablement, and health at its existing stages.

Nonempty `input.ID` replaces only Command ID generation. Correlation IDs and deadlines remain device-owned. Duplicate Command IDs must fail creation with zero dispatch, not return or replay the prior attempt. Existing direct callers construct `CommandInput` with an empty ID. Update every in-repository caller and fake directly; no legacy signature or compatibility wrapper.
...
At each Step, reserve a fresh ID with `devices.NewCommandID`, atomically transition pending → running with the reserved ID, and commit before calling `ExecuteCommand`. This is deliberately two transactions: durable Step intent first, then existing durable Command creation. A crash cannot create an unattributable Command because its ID was already reserved by the Step. Do not foreign-key `reserved_command_id` to a Command that does not yet exist. Resolve actual Command existence/result using a read-only join on that ID when constructing Run details; this also covers a crash after Command creation but before Step result persistence.

On successful outcome, commit Step status and then continue. On an execution error, inspect the reserved ID through `AutomationCommandRecords.GetCommand` before deciding the Run's result: `CommandExecutionError` alone is not proof of a durably terminal Command, because the current lifecycle also uses it for persistence failures. A durable terminal Command record is authoritative: map satisfied/dispatched to success, and other terminal statuses to failure. `ErrCommandNotFound` establishes that no Command exists and permits a failed Step with no Command ID. A nonterminal record or any other read error is uncertain execution, not an ordinary failed Run.

On established failure, commit the failing Step, mark subsequent Steps not_attempted, and mark the Run failed. Known Command failure codes remain intact; pre-creation failures use `invalid_command`, `entity_not_found`, or sanitized `internal_error`. Unexpected errors never leak SQL, credentials, or adapter payloads. If execution is uncertain or result persistence fails, latch an executor fault: atomically close new Run and next-Step admission, report degraded readiness, retain affected active-Run claims, and require restart recovery. Never issue the next Step or silently mark an uncertain Run terminal. Other already-started Commands may finish and persist their known outcomes but cannot start another Step. Scheduler health recovery cannot clear this latch.
...
- `automation_run_steps`: PK `(run_id, step_index)`, FK to Run with cascade on history pruning, `definition_json`, `status`, nullable unique `reserved_command_id`, nullable `outcome`, nullable `failure_code`, nullable `started_at`/`completed_at`. Actual Command status and ID are read from the devices Command table via reserved identity; do not copy full Command observations or pretend a reserved ID proves existence.
...
│   └── devices/
│       ├── model.go, command.go                # modify — CommandInput and shared validation
...
|---|---|---|---|---|---|
| E1 | Reserved-ID Command input and no-side-effect validation; all callers updated | M | `devices/model.go`, `command.go`, `api/`; `devices/`, `app/hearthd/`, `app/zigbee2mqtt/` caller tests | — | A1 |
...
- **A1 — Identity before effect:** inside a fake Command sender, a separate read sees the running Step's reserved ID matching the actual persisted Command ID. Duplicate supplied ID or failed Step-intent commit means zero sends. Invalid target/parameters creates no Command. Direct callers retain existing outcomes and cancellation behavior.
...
- **A7 — Recovery:** crash windows before Command creation, after Command creation, and after Command completion all retain attribution and interrupt the Run with zero restart dispatch. Already terminal Step evidence remains; pending Steps are not_attempted. Test through real SQLite plus embedded NATS, not only fake repository calls.
...
- **A10 — HTTP/OpenAPI:** real Echo/Huma requests establish routes, bodies, status/error codes, required idempotency and revision inputs, disabled default, asynchronous response, and sanitized errors. No endpoint exposes caller-assigned Command IDs.
...
- Reserved intent may exist without a Command after validation/crash: expose the distinction explicitly and test the join; never manufacture a Command record to fill an audit gap.
