...
Command records, transitions, projected State details, and linked waiter results are private implementation types; ingress receives only the durable `ObservationReceipt`. `EntitySupport`, `Value`, and `CommandParameters` each contain one complete valid JSON value; Command parameters are objects. They are normalized and copied defensively at module edges. The module never determines State equality or Command satisfaction by comparing encoded bytes; it delegates both to the Entity-type catalog so object key ordering and future type-specific normalization cannot change semantics.
...
The catalog and its typed authoring helpers are private implementation details constructed exactly once by `devices.New`. JSON Schemas in public `entitytypes` packages remain authoritative for support, State, and parameters. Private generic factories retain compile-time Go types, then erase definitions behind catalog closures so transport and persistence continue to carry generic JSON.

Construction rejects empty or duplicate type IDs, duplicate or non-subject-safe operation names, missing codecs/functions, and non-positive deadlines. The generated built-in catalog supplies power/v1 and brightness/v1 definitions; private same-package tests may construct focused typed catalogs without enabling runtime type loading.
...
An operation key's presence means the Entity supports it. Private Command resolution decodes current typed support, selects operation support, validates typed parameters against both schemas and cross-document rules, and returns normalized parameters plus the immutable ten-second deadline. State normalization applies State schema and current-support compatibility. Equality schema-decodes both values but applies current-support compatibility only to the incoming value, allowing comparison with a previously valid persisted State after support narrows.
...
Outcome satisfaction deliberately excludes current support. It decodes the recorded normalized parameters and candidate State, then applies the immutable operation outcome callback; power/v1 uses `parameters.Value == bool(state)` and brightness/v1 uses exact integer equality. Combined with immutable type IDs/definitions and persisted absolute deadlines, this preserves active and historical Command meaning when re-registration replaces support. Entity `type_id` remains immutable; valid names, external IDs, and normalized support may change transactionally.
...
The app owns the migrated database and its lifetime. `New` atomically interrupts active persisted Commands and prunes expired unpinned receipts before NATS connection. `Run` installs delivery exactly once, makes product methods operational, owns periodic pruning, and drains tracked work on shutdown. Calls racing startup wait for `Run` or their context; calls after stopping return `ErrServiceStopped`. SQLite, catalog construction, clocks, IDs, deadlines, and schedules are private implementation details; only Command delivery has production and in-memory adapters at a real seam.
...
8. Remove the waiter at completion. Graceful and ungraceful shutdown leave unfinished records active; the next `New` marks them `interrupted` and never redispatches them.
...
Owner: `internal/platform/db` for database opening, migrations, query sources, and generated code; behavior-prefixed files in `internal/modules/devices` own concrete SQLite mapping and transactions.
...
`sqlc.yaml` has one SQLite generation entry per query directory, all using the same migrations and emitting to distinct packages under `internal/platform/db/sqlc`. The concrete `devices.Service` implementation composes those generated packages; transactions bind every package they need to the same SQLite transaction. Generated interfaces and types remain private persistence details and are not combined into one generated `Querier`.
...
Registration, Observation, and Command behavior call private concrete SQLite functions beside their owning behavior; no repository or generic transaction seam exists. Generated sqlc types never cross the module interface. Observation receipt insertion, State projection, and matching active Command satisfaction commit in one transaction. Registration stores catalog-normalized `support_json` in its transaction; re-registration replaces that document atomically.
...
3. Call `devices.New` with the app-owned database; construct the private catalog and atomically interrupt active Commands plus prune expired unpinned receipts.
4. Connect to NATS and idempotently provision/validate the stream and durable consumer.
5. Construct the module-local NATS Command delivery adapter and launch `Service.Run` on a dedicated context.
6. Start Registration and Observation ingress through module-local NATS mapping adapters.
7. Construct the Echo/Huma transport with its narrow Entity reader and Command executor interfaces.
8. Listen on loopback.

Shutdown quiesces ingress, cancels and joins `Service.Run`, joins ingress callbacks and HTTP shutdown, then drains NATS and closes SQLite.
...
├── modules/
│   └── devices/
│       ├── api/                     # new — Huma operations, narrow consumed interfaces, HTTP mapping
│       ├── nats/                    # new — domain-aware Registration/Observation/Command mapping
│       ├── model.go                 # new — Device, Entity, generic State value, and Command interface types
│       ├── catalog.go               # new — private closed Entity-type definitions and semantic policies
│       ├── service.go               # new — root construction, operation, maintenance, and lifecycle
│       ├── registration*.go         # new — registration behavior and concrete SQLite transaction
│       ├── observation*.go          # new — Observation/State behavior and concrete SQLite transaction
│       ├── command*.go              # new — durable lifecycle, SQLite transitions, and in-memory waits
│       └── errors.go                # new — domain and lifecycle errors
├── adapters/
...
| Layer | What | How |
| --- | --- | --- |
| Device / Entity module | ID validation, private typed catalog behavior, registration conflicts, receive-order projection, source-time diagnostics, same-value advancement, Command transition monotonicity, concurrent waiter isolation, interleaved outcomes, deadlines, lifecycle, recovery, and pruning | Exercise the concrete `Service` interface with migrated SQLite and an in-memory Command delivery adapter; private controls provide deterministic nondeterminism |
| Persistence invariants | Transactions, constraints, idempotency, receive ordering, receipt pruning/current-State retention, Command lifecycle persistence, atomic outcome satisfaction, startup interruption | Inspect only facts absent from the module interface in temporary real SQLite |
