...
- Revision: `574faef` plus D6 work (reshaped fixtures, `TestFixtureCatalogProbes`, this evidence). All 8 faults ran in an isolated `/tmp` copy of the D6 worktree carrying the uncommitted D6 changes; the working checkout was never mutated for faults. Compiler faults mutate renderer logic and regenerate via `go generate ./entitytypes/`; every faulty output still compiled (`go build ./...` clean), so detection is executable behavioral failure, never drift or compile errors. Each fault was reverted and regenerated to green before the next. After the last fault, `diff -rq` (excluding `.git`) between the isolated copy and the worktree is empty: temporary mutations removed, generation deterministic.
...
- Fault 7, wire a wrong outcome matcher (final variant, not deadline): in `behavior.go` `satisfactionCondition`, negate the conjunction; regenerate. Fails all five operation-bearing wiring subtests with flipped pairs (e.g. `catalog set satisfied outcome = false` plus `catalog set unsatisfied outcome = true`); contract tests fail as well since the full matrix is retained at that seam.
- Fault 8, lose normalized value in the shared helper (final variant, not timestamp): in handwritten `sdk/adapter/typed/entity_construction.go` `NewTypedEntityObservation`, bypass `stateCodec.Encode` with plain `json.Marshal` so the Observation value is no longer schema-validated or normalized (`validateState` and timestamp handling kept); no regeneration. Fails facade `TestGeneratedObservationConformance` typed-invalid acceptance (`colorhsv1/hs-target` example 3, `colormodev1/reported-modes` example 4, `colorxyv1/xy-target` example 3, `temperaturev1/fixed-range-milli-celsius` examples 4–5) and handwritten `TestTypedEntityObservationValidationOrder/schema-invalid_State_fails_after_support_validation`. Brightness/colortemp/power are unaffected: their typed-invalid reps are support-level and still caught by `validateState`.
...
- Assumptions: fixture reorder/outcome-value changes are generator-testdata authoring, not built-in example changes (built-in examples and flags byte-unchanged); Fault 8 drops codec validation and normalization together because no pure-normalization assertion exists (metadata tests compare canonicalized JSON, masking raw-versus-normalized bytes), so typed-invalid acceptance is the honest signal; the routing-swap fault is fixture-only by construction since every built-in has at most one operation; the D2 `3488`-line baseline used a wider file set, so D6 reports explicit group sums instead. Pre-existing `stash@{0}` (autostash) was left untouched.
