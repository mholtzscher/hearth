...
### PR review follow-up: supported generator edge cases

- Full-range support integers now yield no invalid typed mutation instead of wrapping `MinInt64 - 1`; traversal continues to constrained leaves. Restoring the old arithmetic in an isolated copy fails `TestInvalidLeafFullInt64RangeReportsNoMutation`, `TestInvalidLeafSkipsFullRangeLeafForConstrainedLeaf`, and the full-range boundary subtest.
- Unknown-operation probes now choose a name absent from the complete operation set. The fifth executable fixture, `fixtureunknownv1`, declares both `unknown` and `unknown-operation` with compatible parameter schemas. Restoring the old selection loop fails its generated `TestGeneratedCommandConformance/both`: `command for unknown operation was accepted` and `misrouted command invoked a handler: calls = 3, want 2`.
- Catalog selection and generated assertions compare JSON numbers by exact rational value, including nested values and signed zero, without float64 precision loss or number/string conflation. The required fixture uses `8e1`, `75.0`, and `7.5e1`; restoring the old helpers fails `TestGeneratedBuiltinCatalogWiring/fixture.required/v1` on normalized support, State, parameters, and unequal-State selection. Independent unit cases also distinguish integers beyond float64 precision.
- All regression faults were isolated from the working tree. `mise run validate` and `mise run test-entitytype-fixtures` pass. Built-in examples and product behavior remain unchanged; only generator-test fixtures gain edge-case authoring. The generated catalog test now contains 323 lines (31 additional lines for exact numeric comparison); the earlier D6 footprint records the pre-review checkpoint.

### Second PR review follow-up

- Unequal-State probes now search later cases before omitting equality coverage, retaining the authored-supported incoming value and schema-decoded persisted candidate. An isolated catalog equality-always-true fault fails `TestGeneratedBuiltinCatalogWiring/fixture.optional/v1`.
- SDK JSON assertions now retain exact numeric values with `UseNumber` and rational canonicalization. The sixth executable fixture, `fixtureprecisionv1`, uses values above 2^53. An isolated lossy Observation round-trip fails `TestGeneratedObservationMetadata` (`9007199254740992` versus expected `9007199254740993`); restoring the old assertion helper also fails generated numeric comparison tests.
- Invalid-support mutations accept exact rational integer bounds, including exponent and fractional bounds, with int64 range guards. Restoring `ParseInt` fails exponent/fractional boundary regressions. `number` bindings remain unsupported; no schema support was added.
- Example paths are cleaned before embedding; unsupported pattern/filename characters, symlink components, and nested Go modules are rejected. The precision fixture uses `./examples.json` and compiles. Restoring raw path storage produces the expected compile-time `invalid pattern syntax` failure, not a semantic fault claim. Rejection tests cover colon filenames and symlinks.
- A case with no supported operations explicitly tests construction with `Handlers{}`. An isolated generated-wrapper no-op shortcut fails `TestGeneratedCommandConformance/disabled`.
- Faults ran only in an isolated copy, restored to green afterward. `mise run validate` and `mise run test-entitytype-fixtures` pass. Assumptions remain unchanged: built-in examples and public facade semantics are preserved; testdata may exercise additional supported authoring shapes.
