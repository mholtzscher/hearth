...
Registration uses Device kind `light`, `relay`, or `sensor`, generated `powerv1`, `brightnessv1`, `colortempv1`, `colorxyv1`, `colorhsv1`, `colormodev1`, `measurementv1`, `numericsensorv1`, `binarysensorv1`, `enumsettingv1`, `numericsettingv1`, `enumactionv1`, and `enumeventv1` descriptors, and additive Core reconciliation. Re-registration updates names, external IDs, and normalized support without changing canonical IDs. Reconciliation sends the coordinator an immutable MQTT route snapshot.
...
- [ ] Ambient temperature, humidity, illuminance, and battery values register as `hearth.measurement/v1` with their literal measurement kind and canonical unit: temperature preserves Celsius JSON numbers, humidity and battery report finite percent State in 0–100, and illuminance reports finite lux State under a 0–1000000000 kind envelope, with fractions preserved. Occupancy decodes exactly the declared `value_on`/`value_off` scalars to boolean State while every other value stays a per-property decode issue that leaves siblings intact.
...
- [ ] Celsius JSON numbers register as `measurement/v1` kind `temperature` with canonical unit `Cel` and preserve their numeric value; non-number, non-finite, and out-of-range values are rejected.
...
- [ ] Retained replay, pre-dispatch State, stale routes or connection generations, wrong endpoints, wrong normalized brightness, wrong mireds, wrong measurement values, and wrong-mode exact coordinates cannot satisfy a Command.
