...
| D5 adapter settings (3 exposes) | M | D3 | `discovery_wire.go` values/presets parsing; linkquality/startup/power discovery, decoders, translators; the embedded light/linkquality profiles and closed strategies own composition |
...
├── profile_plan.go                          # evaluate groups, gates, dependencies, and device entities
├── profile_strategy.go                      # closed typed State/Command strategy behavior
├── profiles/light.profile.json              # startup/power/effect mapping under the light power gate
├── profiles/linkquality.profile.json        # device-wide unique link-quality mapping
├── device_planner.go                        # merge ordered profile contributions
...
├── entity_electrical.go                     # generic numeric-sensor decode used by link quality
├── entity_startupcolortemp.go               # startup decode/translate (65535 ↔ previous)
