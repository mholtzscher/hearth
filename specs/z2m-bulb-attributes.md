...
| D9 docs + ADR | S | D4–D8 | `README.md`, `specs/generated-entity-type-behavior.md`, unnumbered draft ADR under `docs/adr/` |
...
`hearth.startupcolortemp/v1` (semantic startup temperature). Single `set` operation; public form hides wire 65535. `mode` required; `mireds` optional within the adopted `hearthColorTempMinimum/Maximum` 100–1000 envelope; `oneOf` ties `const` modes to required/forbidden `mireds`. Support `{"state": {"minimum": 142, "maximum": 454}, "operations": {"set": {}}}`. Bounds validated per-entity by `gte_if_present`/`lte_if_present`; satisfaction by `eq` (required `mode`) + `eq_optional` (optional `./mireds`, exact mired equality). Adapter-local wire mapping only: `65535` ↔ `{"mode":"previous"}`, else integer N ↔ temperature, range-checked against discovered bounds; applies only when presets validate a `previous`↔65535 entry (§Interfaces), else the feature is omitted.

```json
{"type": "object", "additionalProperties": false,
 "required": ["mode"],
 "properties": {
   "mode": {"type": "string", "enum": ["temperature", "previous"]},
   "mireds": {"type": "integer", "minimum": 100, "maximum": 1000}},
 "oneOf": [
   {"required": ["mireds"], "properties": {"mode": {"const": "temperature"}}},
   {"not": {"required": ["mireds"]},
    "properties": {"mode": {"const": "previous"}}}]}
```

`hearth.enumaction/v1` (generic stateless enum action; effects). Not an enum setting: a setting holds a persistent choice completed by fresh matching observation; an action requests an effect with no reported state, completed as unverified dispatch. One merged enum-control type would need per-entity statefulness/outcome policies instead of type-level catalog policy; two small generic types avoid that variability and avoid pretending an effect has state. `Values` dynamic from expose `values`; parameter validation reuses `in`. State schema is the closed empty object (accepted by `requireClosedObject`, temperature/power precedent; codegen has no `null` support). Operation `trigger`, deadline 10000 ms, `"outcome": "dispatched"`, `satisfied_when: []`. Statelessness explicit via manifest `"stateless": true` (default false); the adapter registers no decoder and claims no state properties.
...
- `in`: left scalar string ∈ right string array. Right must be non-optional with `minItems ≥ 1` + `uniqueItems`, checked at compile time from its schema; codegen emits a membership loop.
- `eq_optional`: true iff both sides absent, or both present and equal; false on one-sided absence or present-but-unequal. Absence participates (no skip-when-absent). Required `mode` uses `eq`; optional `./mireds` uses `eq_optional` (either side may be the optional leaf; intermediate segments required).
- `gte_if_present`/`lte_if_present`: LEFT must end in an optional LEAF (intermediates required); RIGHT fully required; vacuous pass when the leaf is absent. `null` never reaches rules (schemas admit no null; codec decode rejects it first). The `compileReferencePath` "traverses optional property" rejection in `behavior.go` stays default for all other ops/segments.
- `support_validation`: rule array with roots restricted to `{support}`. `loadModel` compiles it; codegen emits `ValidateSupport(support Support) error` into `zz_generated_behavior.go`; `EntityTypeDefinition.normalizeSupport` invokes it after codec decode, covering registration (`normalizeRegistration` → `NormalizeSupport`) and the command path (`ResolveCommand` re-decodes support). Example: `gte support/state/maximum support/state/minimum`, so `minimum > maximum` and empty/duplicated choices fail at core registration from any adapter.
- Numbers: no DSL change. `eq`/`gte`/`lte` already accept matching-kind `number` operands (`compileEqualityRule`/`compileOrderedRule`); codegen already emits `float64(...)` comparisons (`ruleOperand`) and `==` equality (`schemaComparable`). `numericsensor` state range uses plain `gte`/`lte` (number–number). Per-entity integer-only is NOT a DSL rule — the DSL has no conditional validation, so linkquality integrality stays adapter decode policy (§Interfaces).
...
- `linkquality`: device-root numeric expose, publish bit required + set bit forbidden (`access & 1 != 0 && access & 2 == 0`); plans a `hearth.numericsensor/v1` entity (diagnostic use, no diagnostic metadata); no translator, no `GetProperties` (temperature precedent). Decode admits exact integers 0–255 only — fractional or out-of-range payloads are per-property `stateDecodeIssue`s with siblings intact (adapter-enforced integer constraint; core backstops range via support bounds). Support `{"state": {"minimum": 0, "maximum": 255, "unit": "lqi"}}`. Unit `""` → `"lqi"` (explicit known mapping in code) or `"lqi"` passthrough; any other unit omits the entity. Device-kind-agnostic supplement; never gates/joins the power family.
- `color_temp_startup`: nested `light`-feature numeric, `access & 7 == 7`; bounds from exact-integer `value_min`/`value_max` (142/454) within 100–1000, `minimum < maximum`; presets MUST validate `previous`↔65535 or the feature is omitted. Decode `65535` → previous, else in-range integer → temperature, else per-property `stateDecodeIssue`. `set` previous → `65535` / temperature → mireds as `{"color_temp_startup": N}` + `publishGet` refresh (`reconcile.go`). Planned in `planner_light.go` as optional sibling of a power-eligible root (`validPowerFeature` gates the family; siblings validated independently). Key `startupcolortemp`, external ID `<ieee>/<root|epN>/startupcolortemp` (`scopedIdentity`/`entityLocation`).
...
├── startupcolortempv1/                      # new — temperature-or-previous semantic type
...
├── behavior.go                              # modify — in/eq_optional/gte_if_present/lte_if_present compilation
...
- [ ] A1 `mise run generate` + `mise run validate` clean; catalog holds all four types with §Types supports, 10 s deadlines, `in`/optional rules, dispatched `trigger`; `numericsensor` state bound to `float64` with number–number `gte`/`lte` state validation.
- [ ] A2 Sanitized Wanda-shape fixture discovers linkquality (0–255 `lqi`), startup (142–454), power setting (4 choices), effect (8 values); fractional linkquality payloads rejected at adapter decode with siblings intact; unknown/`start_bind` siblings ignored; malformed sibling suppresses nothing valid.
- [ ] A3 `65535` decodes to `{"mode":"previous"}`; `250` → temperature; `455` against 142–454 support is a property issue with siblings intact.
- [ ] A4 Schema `oneOf` rejects `{"mode":"temperature"}` without mireds and `{"mode":"previous","mireds":250}`.
- [ ] A5 `in` rejects off-choices state/parameters (e.g. `"eco"`); empty/duplicated choices rejected at core registration via schema bounds.
- [ ] A6 Decoded binary64 `min>max` support rejected at core registration, not only in adapter planning; fractional in-range state (e.g. `18.5` under 0–255 support) passes core `numericsensor` validation, proving the type is genuinely fractional while linkquality integer-only stays adapter decode policy. Tests pin overflow rejection, underflow-to-zero, and equal decoded bounds for wire minimum `9007199254740993` / maximum `9007199254740992`.
...
- [ ] A8 Core rejects effect observations (even null/empty) with `invalid_value` and no stored value; `GET` entity state stays `null`; rejection visible in history reads, distinct from state history.
...
- [ ] A12 `eq_optional` truth table (absent/absent true; one-absent false; present-equal true; present-unequal false); JSON null rejected at codec decode; mireds checked against discovered 142–454, not just 100–1000; wrong-membership commands fail pre-dispatch in `ResolveCommand`.
...
| Layer | What | How |
|-------|------|-----|
| Unit (codegen) | `in` membership, `eq_optional` truth table, `gte/lte_if_present` vacuous-absent, `support_validation` min>max, number→`float64` binding + number `gte`/`lte` emission, dispatched/no-predicate + empty-object state | Generator unit tests in D1–D2 before entity types; `entitytypegen` golden tests; `TestTypeEmitterRejectsLossyNumberBindings` revised to the new binding |
...
| Integration (adapter) | Eligibility matrix (access/units/presets/uniqueness), linkquality integer-only decode (fractions rejected, siblings intact), 65535 mapping, off-choices rejection, sibling isolation | Planner/decoder tests over sanitized Wanda-shape + adversarial fixtures |
...
| Number binding is lossy binary64 (revises the lossy-binding guardrail) | Medium | Medium | Integers through 2^53 exact (covers linkquality 0–255); validation/equality use rounded binary64 values; overflow rejected, underflow may become zero; §Types states the precision contract; golden tests pin the new binding |
...
| `float64` state binding | Exact-decimal (`json.Number`/`big.Rat`) binding | `ruleOperand` already emits `float64(...)` and `schemaComparable` already covers `number`, so comparisons and `EqualState` work with zero DSL change; exact-decimal would need new comparison helpers for no read-only-sensor benefit. Retain `float64` |
| Adapter-enforced integer-only for linkquality | New conditional DSL op / `multiple_of` on numbers | DSL has no conditionals, so per-entity integrality cannot be expressed without breaking fractional use; float modulo brings rounding hazards. Adapter decode rejects fractions, core backstops range. Retain |
...
| `eq_optional` leaf comparison | Wiring generated `EqualState` into satisfaction | Narrower generator change for optional `./mireds` exact-equality; `EqualState` wiring replaces rather than eliminates a generator extension. Retain |
...
| Single startup `set` with tagged `previous` | Separate mode + temperature operations | One operation, honest optional state, schema-enforced shape; wire 65535 stays adapter-local |
