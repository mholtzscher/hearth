...
type StateSupport = contractpowerv1.StateSupport
type OperationSupport = contractpowerv1.OperationSupport
type SetSupport = contractpowerv1.SetSupport
...
func NewEntityDescriptor(metadata adapter.EntityMetadata, support Support) (adapter.EntityDescriptor, error) {
	...
	descriptor, err := typed.NewTypedEntityDescriptor(metadata, contractpowerv1.TypeID, support, codecs.Support)
	if err != nil {
		return adapter.EntityDescriptor{}, err
	}
	if err := contractpowerv1.ValidateSupport(support); err != nil {
...
func NewCommandHandler(entityID string, support Support, handlers Handlers) (adapter.CommandHandler, error) {
	...
	setRoute, err := typed.Operation(entityID, contractpowerv1.OperationSet, func(raw json.RawMessage) (SetParameters, error) {
		parameters, _, err := codecs.SetParameters.Decode(raw)
		if err != nil {
			return SetParameters{}, err
		}
		if err := contractpowerv1.ValidateSetParameters(support, setSupport, parameters); err != nil {
			return SetParameters{}, err
		}
...
func NewObservation(input ObservationInput) (adapter.Observation, error) {
	...
	return typed.NewTypedEntityObservation(typed.EntityObservationInput[State, Support]{EntityID: input.EntityID, Support: input.Support, State: input.State, AdapterReceivedAt: input.AdapterReceivedAt, SourceUpdatedAt: input.SourceUpdatedAt}, codecs.State, codecs.Support, contractpowerv1.ValidateState)
}

func codecs() (*contractpowerv1.Codecs, error) {
	compileOnce.Do(func() { sharedCodecs, compileErr = contractpowerv1.Compile() })
	if compileErr != nil {
