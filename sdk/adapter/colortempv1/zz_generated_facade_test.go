...
func TestGeneratedObservationConformance(t *testing.T) {
	...
	t.Run("fixture-153-500-step-1", func(t *testing.T) {
		...
		{
			raw := json.RawMessage("{\"active\": true, \"value\": 370}")
		...
		{
			raw := json.RawMessage("{\"active\": false, \"value\": 370}")
			state, _, codecErr := codecs.State.Decode(raw)
			if codecErr != nil {
		...
		{
			raw := json.RawMessage("{\"active\": true, \"value\": 153}")
			state, _, codecErr := codecs.State.Decode(raw)
			if codecErr != nil {
		...
		{
			raw := json.RawMessage("{\"active\": true, \"value\": 500}")
			state, _, codecErr := codecs.State.Decode(raw)
			if codecErr != nil {
		...
		{
			raw := json.RawMessage("{\"active\": true, \"value\": 152}")
			state, _, codecErr := codecs.State.Decode(raw)
		...
		{
			raw := json.RawMessage("{\"active\": true, \"value\": 501}")
			state, _, codecErr := codecs.State.Decode(raw)
			var plainState State
			plainErr := json.Unmarshal(raw, &plainState)
			if codecErr == nil {
...
func TestGeneratedCommandConformance(t *testing.T) {
	...
	t.Run("fixture-153-500-step-1", func(t *testing.T) {
		...
		if err := handler(context.Background(), adapter.Command{ID: "cmd_set_one", CorrelationID: "cor_set_one", EntityID: "ent_01890f47-7a6b-7c4d-8e9f-0123456789ab", OperationName: "set", Parameters: json.RawMessage("{\"value\": 370}"), Deadline: deadline.Format(time.RFC3339Nano)}, nil); err != nil {
		...
		{
			if _, _, decodeErr := codecs.SetParameters.Decode(json.RawMessage("{\"value\": 370}")); decodeErr != nil {
				t.Errorf("set parameter example 1 marked valid but rejected by parameters codec: %v", decodeErr)
			}
		}
		{
			raw := json.RawMessage("{\"value\": 152}")
			before := len(setCalls)
		...
		{
			before := len(setCalls)
			if invokeErr := handler(context.Background(), adapter.Command{ID: "cmd_wrong_entity", CorrelationID: "cor_wrong_entity", EntityID: "ent_6ba7b810-9b21-11d1-80b4-00c04fd430c8", OperationName: "set", Parameters: json.RawMessage("{\"value\": 370}"), Deadline: deadline.Format(time.RFC3339Nano)}, nil); invokeErr == nil {
				t.Error("command for wrong entity ID was accepted")
			}
			if invokeErr := handler(context.Background(), adapter.Command{ID: "cmd_unknown_operation", CorrelationID: "cor_unknown_operation", EntityID: "ent_01890f47-7a6b-7c4d-8e9f-0123456789ab", OperationName: "unknown", Parameters: json.RawMessage("{\"value\": 370}"), Deadline: deadline.Format(time.RFC3339Nano)}, nil); invokeErr == nil {
