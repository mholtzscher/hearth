...
func renderFacadeConformanceTest(model entityTypeModel) (output, error) {
	...
	for _, example := range model.Examples.Cases {
		...
		for index, state := range example.States {
			...
			fmt.Fprintf(
				&source,
				"\t\t\tstate, _, decodeErr := codecs.State.Decode(json.RawMessage(%s))\n",
			...
			fmt.Fprintf(
				&source,
				"\t\t\tif valid != %t { t.Errorf(%s, valid, decodeErr, observationErr) }\n",
				state.Valid,
				strconv.Quote(
					fmt.Sprintf("State example %d: valid = %%v, decode error = %%v, Observation error = %%v", index+1),
				),
			)
			source.WriteString("\t\t}\n")
		}
