...
func renderFacade(model entityTypeModel) ([]byte, error) {
	...
	fmt.Fprintf(&source, "\tcontract%s \"github.com/mholtzscher/hearth/entitytypes/%s\"\n", model.Package, model.Package)
	source.WriteString("\t\"github.com/mholtzscher/hearth/sdk/adapter\"\n\t\"github.com/mholtzscher/hearth/sdk/adapter/typed\"\n)\n\n")
	fmt.Fprintf(&source, "type Support = contract%s.Support\ntype State = contract%s.State\n", model.Package, model.Package)
	fmt.Fprintf(&source, "type StateSupport = contract%s.StateSupport\ntype OperationSupport = contract%s.OperationSupport\n", model.Package, model.Package)
	for _, operation := range model.Operations {
		fmt.Fprintf(&source, "type %sSupport = contract%s.%sSupport\n", operation.GoName, model.Package, operation.GoName)
		fmt.Fprintf(&source, "type %sParameters = contract%s.%sParameters\n", operation.GoName, model.Package, operation.GoName)
	...
	source.WriteString("}\n\n")
	source.WriteString("type ObservationInput struct {\n\tEntityID string\n\tSupport Support\n\tState State\n\tAdapterReceivedAt time.Time\n\tSourceUpdatedAt *time.Time\n\tRefreshForCommand *string\n}\n\n")
	source.WriteString("var (\n\tcompileOnce sync.Once\n")
	fmt.Fprintf(&source, "\tsharedCodecs *contract%s.Codecs\n", model.Package)
	source.WriteString("\tcompileErr error\n)\n\n")
	source.WriteString("func NewEntityDescriptor(metadata adapter.EntityMetadata, support Support) (adapter.EntityDescriptor, error) {\n\tcodecs, err := codecs()\n\tif err != nil { return adapter.EntityDescriptor{}, err }\n\tnormalized, err := codecs.Support.Encode(support)\n\tif err != nil { return adapter.EntityDescriptor{}, validationError(fmt.Errorf(\"invalid Entity support: %w\", err)) }\n")
	fmt.Fprintf(&source, "\treturn adapter.EntityDescriptor{Key: metadata.Key, ExternalID: metadata.ExternalID, Name: metadata.Name, Type: contract%s.TypeID, Support: normalized}, nil\n}\n\n", model.Package)

	if len(model.Operations) > 0 {
		source.WriteString("func NewCommandHandler(entityID string, support Support, handlers Handlers) (adapter.CommandHandler, error) {\n\tcodecs, err := codecs()\n\tif err != nil { return nil, err }\n\tif _, err := codecs.Support.Encode(support); err != nil { return nil, validationError(fmt.Errorf(\"invalid Entity support: %w\", err)) }\n\tvar routes []typed.Route\n")
		for _, operation := range model.Operations {
			if operation.Required {
				fmt.Fprintf(&source, "\tif handlers.%s == nil { return nil, validationError(errors.New(%s)) }\n", operation.GoName, strconv.Quote(operation.Name+" handler is required"))
...
func writeRoute(source *strings.Builder, model entityTypeModel, operation operationModel, indent, supportVariable string) {
	variable := lowerFirst(operation.GoName) + "Route"
	fmt.Fprintf(source, "%s%s, err := typed.Operation(entityID, contract%s.Operation%s, func(raw json.RawMessage) (%sParameters, error) {\n", indent, variable, model.Package, operation.GoName, operation.GoName)
	...
	fmt.Fprintf(source, "%s\tif err := contract%s.Validate%sParameters(support, %s, parameters); err != nil { return %sParameters{}, err }\n", indent, model.Package, operation.GoName, supportVariable, operation.GoName)
