...
func generateRoot(root string, check bool) error {
	...
	catalog := renderCatalog(models, modulePath, absoluteRoot)
	catalogTest, err := renderCatalogConformanceTest(models, modulePath, absoluteRoot)
	if err != nil {
		return fmt.Errorf("render built-in catalog conformance test: %w", err)
	}
	outputs = append(outputs, catalog, catalogTest)
	formatted, formatErr := formatGeneratedOutputs(outputs)
	if formatErr != nil {
		return formatErr
	}
	return applyOutputs(absoluteRoot, formatted, check)
}
