...
// This test protects persisted eligibility through the new entry point and
...
func TestPruneHistoryKeepsCurrentStateAnchorAndFreshEntityEvents(t *testing.T) {
	...
	project := func(observation Observation, observedAt time.Time) {
		t.Helper()
	...
	record := func(event EntityEvent, receivedAt time.Time) {
		t.Helper()
