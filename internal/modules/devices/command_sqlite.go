...
func (service *Service) createCommand(ctx context.Context, command commandRecord) error {
	if command.status != commandStatusRequested || command.acceptedAt != nil || command.completedAt != nil ||
		command.outcomeObservationID != nil || command.failureCode != nil {
...
func (service *Service) markCommandAccepted(ctx context.Context, id CommandID, acceptedAt time.Time) error {
	...
	if err != nil {
		return fmt.Errorf("begin Command acceptance: %w", err)
	}
	defer tx.Rollback()
	rows, err := commandsqlc.New(tx).MarkCommandAccepted(ctx, commandsqlc.MarkCommandAcceptedParams{
	...
	if rows == 0 {
		if _, err := getCommand(ctx, tx, id); err != nil {
			return err
		}
		return errCommandTerminal
	}
	if err := tx.Commit(); err != nil {
		return fmt.Errorf("commit Command acceptance: %w", err)
	}
	return nil
}
...
func (service *Service) completeCommand(ctx context.Context, completion commandCompletion) error {
	...
	if command.status != commandStatusRequested && command.status != commandStatusAccepted {
		if sameCompletion(command, completion) {
			if err := tx.Commit(); err != nil {
				return fmt.Errorf("commit repeated Command completion: %w", err)
			}
			return nil
		}
		return errCommandTerminal
