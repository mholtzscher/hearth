...
func Register(api huma.API, service Automations) {
	...
	// Publish the strict recursive Condition family and decision rules after the
	// generated DTO components exist, so every response and the definition body
	// reference one closed oneOf definition instead of a permissive DTO shape.
	// The DTO output mappings stay unchanged.
	publishConditionSchemas(api, codec.AutomationDefinitionSchema())
	publishConditionDecisionSchema(api)
}

// conditionComponentName maps one embedded Condition $defs name to the reusable
// OpenAPI component that publishes it. Publication and reference rewriting share
// it, so every rewritten reference resolves to a published component.
func conditionComponentName(defName string) string {
	switch defName {
	case "condition":
		return "AutomationConditionBody"
	case "entityStateCondition":
		return "AutomationConditionEntityStateBody"
	case "allCondition":
		return "AutomationConditionAllBody"
	case "anyCondition":
		return "AutomationConditionAnyBody"
	case "notCondition":
		return "AutomationConditionNotBody"
	case "conditionID":
		return "AutomationConditionIDBody"
	case "children":
		return "AutomationConditionChildrenBody"
	default:
		return "AutomationCondition" + strings.ToUpper(defName[:1]) + defName[1:] + "Body"
	}
}

// rewriteConditionRefs rewrites the embedded schema's local #/$defs references to
// the published components so the recursive family constraints resolve by plain
// JSON Pointer instead of an $id registry.
func rewriteConditionRefs(value any) any {
			...
			if key == "$ref" {
				if ref, ok := item.(string); ok {
					if name, found := strings.CutPrefix(ref, "#/$defs/"); found {
						rewritten[key] = "#/components/schemas/" + conditionComponentName(name)
						continue
					}
				}
			}
			rewritten[key] = rewriteConditionRefs(item)
		}
		return rewritten
	case []any:
		items := make([]any, len(node))
		for index, item := range node {
			items[index] = rewriteConditionRefs(item)
		}
		return items
	default:
		return value
...
// publishConditionSchemas publishes the strict recursive Condition family from
// the same embedded schema the definition codec compiles, so response schemas and
// request bodies agree on one closed oneOf definition with family-inapplicable
// fields forbidden.
func publishConditionSchemas(api huma.API, definitionSchema json.RawMessage) {
	var document map[string]any
	if err := json.Unmarshal(definitionSchema, &document); err != nil {
		panic(fmt.Errorf("automation condition schema publication: %w", err))
	}
	defs, ok := document["$defs"].(map[string]any)
	if !ok {
		panic("automation definition schema is missing $defs")
	}
	schemas := api.OpenAPI().Components.Schemas.Map()
	for name, raw := range defs {
		value, isObject := rewriteConditionRefs(raw).(map[string]any)
		if !isObject {
			panic(fmt.Errorf("automation condition definition %q is not an object", name))
		}
		schemas[conditionComponentName(name)] = &huma.Schema{Extensions: value}
	}
}

// publishConditionDecisionSchema replaces the permissive generated decision DTO
// schema with the exact mode presence and bypass rules of the product contract.
// The DTO output mapping is unchanged; only the published schema is tightened, so
// callers can see which snapshot and evaluation members each mode requires and
// when a bypass may be requested.
func publishConditionDecisionSchema(api huma.API) {
	api.OpenAPI().Components.Schemas.Map()["AutomationConditionDecisionBody"] = conditionDecisionSchema()
}

// conditionDecisionSchema is the closed decision shape: not_configured carries
// neither snapshot nor evaluation, not_evaluated and bypassed carry a snapshot
// only, evaluated carries both, and bypass_requested is exactly true for the
// bypassed mode and false for every other configured mode. A manual Run may also
// record not_configured with bypass_requested true, so that mode leaves the flag
// unconstrained.
func conditionDecisionSchema() *huma.Schema {
	const (
		modeNotConfigured = "not_configured"
		modeNotEvaluated  = "not_evaluated"
		modeBypassed      = "bypassed"
		modeEvaluated     = "evaluated"
	)
	const (
		modeMember       = "mode"
		snapshotMember   = "snapshot"
		evaluationMember = "evaluation"
		bypassMember     = "bypass_requested"
	)
	modeBranch := func(mode string, bypass *bool, required []string, noEvaluation bool) *huma.Schema {
		properties := map[string]*huma.Schema{modeMember: {Const: mode}}
		if bypass != nil {
			properties[bypassMember] = &huma.Schema{Const: *bypass}
		}
		branch := &huma.Schema{Properties: properties, Required: required}
		if noEvaluation {
			branch.Not = &huma.Schema{Required: []string{evaluationMember}}
		}
		return branch
	}
	falseFlag, trueFlag := false, true
	notConfigured := &huma.Schema{
		Properties: map[string]*huma.Schema{modeMember: {Const: modeNotConfigured}},
		Not: &huma.Schema{AnyOf: []*huma.Schema{
			{Required: []string{snapshotMember}},
			{Required: []string{evaluationMember}},
		}},
	}
	return &huma.Schema{
		Type:                 huma.TypeObject,
		Required:             []string{modeMember, bypassMember},
		AdditionalProperties: false,
		Properties: map[string]*huma.Schema{
			modeMember: {
				Type: huma.TypeString,
				Enum: []any{modeNotConfigured, modeNotEvaluated, modeBypassed, modeEvaluated},
			},
			bypassMember:     {Type: huma.TypeBoolean},
			snapshotMember:   {Ref: "#/components/schemas/AutomationConditionBody"},
			evaluationMember: {Ref: "#/components/schemas/AutomationConditionEvaluationBody"},
		},
		OneOf: []*huma.Schema{
			notConfigured,
			modeBranch(modeNotEvaluated, &falseFlag, []string{snapshotMember}, true),
			modeBranch(modeBypassed, &trueFlag, []string{snapshotMember}, true),
			modeBranch(modeEvaluated, &falseFlag, []string{snapshotMember, evaluationMember}, false),
		},
	}
}
