...
func TestRegistrationServerClosedWaitsForHandler(t *testing.T) {
	...
	request := Envelope[Registration]{
		ID: "reg_01890f47-7a6b-7c4d-8e9f-0123456789ab", Schema: contractsv1.RegistrationRequestSchemaID,
	...
	select {
	case <-closed:
		t.Fatal("Registration server closed before its handler returned")
	default:
	}
	close(release)
	select {
	...
	case <-time.After(time.Second):
		t.Fatal("Registration server did not close after its handler returned")
	}
}
