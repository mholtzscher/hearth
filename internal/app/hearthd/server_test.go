...
func (adapter *testDevices) GetEntity(context.Context, devices.EntityID) (devices.EntityView, error) {
	return adapter.view, nil
}

func (*testDevices) ExecuteCommand(
	context.Context,
	devices.EntityID,
...
func TestHTTPHandlerServesHealthReadinessAndDeviceOperations(t *testing.T) {
	adapter := &testDevices{view: devices.EntityView{Entity: devices.Entity{
	...
	handler, api := NewHTTPHandler(devicesapi.Dependencies{Entities: adapter, Commands: adapter}, readiness)
...
func TestRuntimeOpenAPIContract(t *testing.T) {
	...
	handler, _ := NewHTTPHandler(devicesapi.Dependencies{Entities: adapter, Commands: adapter}, &testReadiness{})
