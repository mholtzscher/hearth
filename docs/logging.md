...
| `device_fact.not_published` | A Device Fact was dropped instead of published, with `family`, the safe source ID (`observation_id`, `event_id` or `command_id`), `stage`, and a fixed `error_code`; Warn for expected loss (queue overflow, closed or reconnecting publication connection) and Error for an internal defect (invalid fact, zero clock, or an ID, subject, size or encoding failure) |
...
Device Facts follow the same rule. `device_fact.not_published` reports a dropped publication and `device_fact.suppressed` reports connection-freshness suppression; neither contains State values, Command parameters, raw envelopes or full subjects, and neither reconstructs durable fact delivery. The durable SQLite record and the HTTP read APIs stay authoritative, so a missing fact—no subscriber connected, publication connection unavailable, or backlog suppressed behind the live epoch—is expected under the at-most-once contract, not a logging failure.
