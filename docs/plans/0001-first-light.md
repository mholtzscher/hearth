...
- Preserve the concrete, stateless SDK `Session` with `Connect`, generic `Register`, durable ordinary `PublishObservation`, blocking ephemeral `ServeCommands`, and idempotent `Close`. A successful `Responder.Accept` returns command evidence that publishes Observations linked to that accepted Command without the handler context; public Observation types carry no raw Command link. Add typed routing and generated power/v1 and brightness/v1 facades that hide raw JSON and operation-name switching from Go adapter consumers.
- Have ordinary publication and command evidence retry one generated envelope through transient disconnects and return only after JetStream acknowledgement or the effective context expires; do not add a local outbox.
- Keep vendor behavior, discovery, upstream calls, refresh rules, credentials, and checkpoints outside the SDK.
- Serve Commands through independent, potentially concurrent SDK handler invocations with one-shot responders: adapter code calls `Accept` before refresh or `Reject`; successful acceptance returns the evidence capability used for each linked refresh. Adapters must be concurrency-safe and may serialize internally only when their vendor protocol requires it.
...
- Retain adapter observations in a file-backed JetStream Limits stream for seven days or one GiB, discard oldest messages, and consume with explicit acknowledgements, a 30-second ack wait, one pending acknowledgement, and unlimited redelivery; retain processed Observation IDs in SQLite for at least eight days and keep the observation backing current State until superseded.
- Store canonical state, identity mappings, Observations, and a minimal Command attempt history transactionally in the core's SQLite database using Goose migrations and sqlc-generated queries; keep generated types behind module persistence adapters.
- Record adapter acquisition time as `adapter_received_at`, optional upstream last-change time as `source_updated_at`, JetStream's server-assigned durable-consumer receive time as core-owned `observed_at`, and an internal SQLite logical sequence as `receive_order`.
...
- Have the disposable Home Assistant adapter subscribe before fetching each startup/reconnect snapshot, buffer and reconcile intervening state-change events by Home Assistant `last_updated`, handle concurrent WebSocket requests by request ID, and use accepted Command evidence to publish each linked refresh.
