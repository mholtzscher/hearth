...
- Each first-seen, valid, correctly owned Observation advances canonical State. A same-value Observation advances its ID/timestamps and is `unchanged`; a different value is `applied`.
...
- Commands for one Entity may overlap; the core does not guard, queue, or supersede them. Each Command has an independent ID, durable record, in-memory waiter, and ten-second deadline, and only its own linked matching Observation can satisfy it. Interleaved Commands may both be satisfied at different receive orders or may cause one another to time out; canonical State still follows core receive order.
- Command delivery and outcome waits are ephemeral and in memory, while SQLite stores a minimal non-replayable record of each attempt and terminal outcome for diagnosis and future history. The record commits as `requested` before dispatch; a matching linked Observation marks only that Command `satisfied` in the same transaction as State projection. After the `requested` record commits, the core proceeds with dispatch and retains that lifecycle until linked outcome or the ten-second deadline even if the HTTP client disconnects. A core restart loses active waits, marks persisted `requested` and `accepted` records `interrupted`, and never redispatches them.
