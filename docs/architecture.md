...
- Command delivery, outcome waits, and one-in-flight-per-Entity enforcement are ephemeral and in memory, while SQLite stores a minimal non-replayable record of each attempt and terminal outcome for diagnosis and future history. The record commits as `requested` before dispatch; a matching linked Observation marks it `satisfied` in the same transaction as State projection. After the `requested` record commits, the core proceeds with dispatch and keeps the guard until linked outcome or the ten-second deadline even if the HTTP client disconnects. A core restart loses the wait and guard, marks persisted `requested` and `accepted` records `interrupted`, and never redispatches them.
